...
impl LibraLoggerBuilder {
    ...
    pub fn central_level(&mut self, level: Level) -> &mut Self {
        self.central_level = level;
        self
    }
    ...
    pub fn build(&mut self) -> Arc<LibraLogger> {
        let filter = {
            ...
            let central_logging_filter = {
                let mut filter_builder = Filter::builder();

                if self.is_async && self.address.is_some() {
                    filter_builder.filter_level(self.central_level.into());
                } else {
...
struct LibraFilter {
    local_filter: Filter,
    central_logging_filter: Filter,
}

impl LibraFilter {
    fn enabled(&self, metadata: &Metadata) -> bool {
        self.local_filter.enabled(metadata) || self.central_logging_filter.enabled(metadata)
    }
}
